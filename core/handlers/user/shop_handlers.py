...
from core.keyboards.inline.shop.high_price import get_high_price_kb
from core.keyboards.inline.shop.low_price import get_low_price_kb
from core.models.product import LowPriceProduct, HighPriceProduct
from core.middlewares.sale_middleware import SaleMiddleware

router = Router(name=__name__)

router.callback_query.middleware(SaleMiddleware())
# router.callback_query.outer_middleware(SaleMiddleware())

@router.message(Command(commands='low_price'))
async def low_price_menu(msg:Message):
    await msg.answer(text='Вот список недорогих товаров:',reply_markup=get_low_price_kb())

@router.message(Command(commands='high_price'))
async def high_price_menu(msg:Message):
    await msg.answer(text='Вот список дорогих товаров:',reply_markup=get_high_price_kb())
...
async def high_price_callback(call:CallbackQuery, callback_data:HighPriceProduct, is_sale:bool):
    name = callback_data.name
    price = callback_data.get_sale_price(is_sale)
    await call.message.answer(text=f'{name} шикарно подойдет вам! С вас {price} алготокенов!')